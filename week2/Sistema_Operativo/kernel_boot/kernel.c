...
void kmain(void) {
    ...
    // Sonda rápida del módulo, pero solo dibujamos el contorno de la barra ahora
    ...
    print_hex(MODULE_LOAD_ADDRESS); print_string(" ", C(THEME_MUTED));
    if (module_present) {
        print_string("SONDEADO", vga_attr(THEME_BG, THEME_OK)); print_string(" ", C(THEME_MUTED));
    ...
    print_string("  PseudoKernel:  ", C(THEME_MUTED)); print_hex(kstart); print_string(" - ", C(THEME_MUTED)); print_hex(kend); print_string(" (", C(THEME_MUTED)); print_dec(ksize); print_string(" bytes)", C(THEME_MUTED)); putchar('\n', C(THEME_MUTED));
    ...
    bool finalized = module_finalize_after_bar();
        print_string("  Copia modulo: ", C(THEME_MUTED)); print_string(finalized?"OK":"FALLO", finalized?vga_attr(THEME_BG, THEME_OK):vga_attr(THEME_BG, THEME_ERR)); newline(C(THEME_TEXT));
        // Título compacto detalles de módulo (reducimos altura)
        print_string("  -- Detalles modulo --", vga_attr(THEME_BG, THEME_ACCENT)); newline(C(THEME_TEXT));
        ...
        uint16_t version=0, length_field=0; uint32_t entry_off=0; bool header_ok = mod0_parse(m,&version,&length_field,&entry_off);
    print_string("  Cabecera: ", C(THEME_MUTED));
        if (header_ok) {
            ...
            if (entry_valid) module_entry_fn = (void(*)(void))((uintptr_t)MODULE_LOAD_ADDRESS + entry_off);
            print_string(" ver=", C(THEME_MUTED)); print_dec(version);
            print_string(" len=", C(THEME_MUTED)); print_dec(length_field);
            print_string(" entry_off=", C(THEME_MUTED)); print_hex(entry_off);
    ...
    newline(C(THEME_TEXT));
    print_string("  Dump[0..15]: ", C(THEME_MUTED)); print_hex_bytes((const uint8_t*)m,16,C(THEME_TEXT),C(THEME_MUTED)); newline(C(THEME_TEXT));
    ...
    print_centered("Pulsa cualquier tecla para ejecutar el modulo", vga_attr(THEME_BG, THEME_ACCENT));
        ...
        if (module_entry_fn) {
            ...
            if (!inside_sector) {
                print_string("  Entry fuera de rango (>=512b): abort", vga_attr(THEME_BG, THEME_ERR)); newline(C(THEME_TEXT));
            } else {
                print_string("  Entry OK -> ", C(THEME_MUTED)); print_hex((uint32_t)(uintptr_t)module_entry_fn); newline(C(THEME_TEXT));
                uint32_t esp_before = read_esp();
                print_string("  ESP antes: ", C(THEME_MUTED)); print_hex(esp_before); newline(C(THEME_TEXT));
                module_entry_fn();
                uint32_t esp_after = read_esp();
                print_string("  ESP despues: ", C(THEME_MUTED)); print_hex(esp_after); newline(C(THEME_TEXT));
                print_string("  Retorno modulo", vga_attr(THEME_BG, THEME_OK)); newline(C(THEME_TEXT));
            }
        } else {
            print_string("  Entry invalido: no se ejecuta modulo", vga_attr(THEME_BG, THEME_ERR)); newline(C(THEME_TEXT));
        }
    ...
    wait_key_or_timeout_ms(3000);
        ...
        print_string("  Modulo opcional:     ", C(THEME_MUTED)); print_hex(MODULE_LOAD_ADDRESS); print_string(" CARGADO", vga_attr(THEME_BG, THEME_OK)); newline(C(THEME_TEXT)); newline(C(THEME_TEXT));
        // Comprobaciones de nuevo
        struct GDTR gdtr2; bool pm2=check_protected_mode(); bool seg2=check_segments_ok(); bool gdt2=check_gdt_ok(&gdtr2); bool a202=check_a20_enabled(); bool vga2=check_vga_access();
        print_separator('-', sep2);
        ...
        newline(C(THEME_TEXT));
        print_separator('-', vga_attr(THEME_BG, THEME_ACCENT));
        uint32_t kstart2=(uint32_t)&_start, kend2=(uint32_t)&_end; uint32_t ksize2=kend2-kstart2;
        print_string("  PseudoKernel:  ", C(THEME_MUTED)); print_hex(kstart); print_string(" - ", C(THEME_MUTED)); print_hex(kend); print_string(" (", C(THEME_MUTED)); print_dec(ksize); print_string(" bytes)", C(THEME_MUTED)); putchar('\n', C(THEME_MUTED));
    print_string("  Stack:         ", C(THEME_MUTED)); print_hex(0x00090000); putchar('\n', C(THEME_MUTED));
    print_string("  VGA:           ", C(THEME_MUTED)); print_hex(0x000B8000); putchar('\n', C(THEME_MUTED));
        newline(C(THEME_TEXT));
